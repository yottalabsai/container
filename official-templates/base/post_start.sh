...
if [[ -z "${YOTTA_PROJECT_ID}" ]]; then
    echo "YOTTA_PROJECT_ID environment variable is not set. Exiting."
    exit 0
fi
    ...
    while true; do
        ...
        if [[ "$connections" -eq 0 ]]; then
            ...
            if [[ "$countdown" -le 0 ]]; then
                echo "Countdown reached zero. Removing pod: $YOTTA_POD_ID"
                yottactl remove pod $YOTTA_POD_ID
                exit 0
            fi
        else
